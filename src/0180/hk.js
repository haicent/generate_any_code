...
function showOPInfo(szInfo, status, xmlDoc) {
  var szTip =
    "<div>" + dateFormat(new Date(), "yyyy-MM-dd hh:mm:ss") + " " + szInfo;
...
function clickSetLocalCfg() {
  ...
  arrXml.push(
    "<DeviceCapturePath>" + $("#devicePicPath").val() + "</DeviceCapturePath>"
  );
...
function clickGetDeviceInfo() {
  ...
  WebVideoCtrl.I_GetDeviceInfo(szDeviceIdentify, {
    success: function (xmlDoc) {
      var arrStr = [];
      arrStr.push(
        "设备名称：" + $(xmlDoc).find("deviceName").eq(0).text() + "\r\n"
      );
      arrStr.push(
        "设备ID：" + $(xmlDoc).find("deviceID").eq(0).text() + "\r\n"
      );
      arrStr.push("型号：" + $(xmlDoc).find("model").eq(0).text() + "\r\n");
      arrStr.push(
        "设备序列号：" + $(xmlDoc).find("serialNumber").eq(0).text() + "\r\n"
      );
      arrStr.push(
        "MAC地址：" + $(xmlDoc).find("macAddress").eq(0).text() + "\r\n"
      );
      arrStr.push(
        "主控版本：" +
...
function clickCapturePicData() {
  var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
    szInfo = "";
  if (oWndInfo != null) {
    WebVideoCtrl.I2_CapturePicData().then(
      function (data) {
        szInfo = "抓图上传成功！";
        console.log(data);
        showOPInfo(oWndInfo.szDeviceIdentify + " " + szInfo);
        ...
        if ("MORE" === $(xmlDoc).find("responseStatusStrg").eq(0).text()) {
          for (
            ...
            objTd.innerHTML =
              ...
              (i + g_iSearchTimes * 40) +
              ");'>下载</a>";
            $("#downloadTd" + (i + g_iSearchTimes * 40)).data(
              "fileName",
              szFileName
            );
            $("#downloadTd" + (i + g_iSearchTimes * 40)).data(
...
function clickStartDownloadRecord(i) {
  ...
  g_iDownloadID = WebVideoCtrl.I_StartDownloadRecord(
    szDeviceIdentify,
    szPlaybackURI,
    szFileName,
    {
...
function clickStartDownloadRecordByTime() {
  ...
  g_iDownloadID = WebVideoCtrl.I_StartDownloadRecordByTime(
    szDeviceIdentify,
    szPlaybackURI,
    szFileName,
    szStartTime,
...
function clickExportDeviceConfig() {
  ...
  var iRet = WebVideoCtrl.I_ExportDeviceConfig(
    szDeviceIdentify,
    szDevicePassWord
  );
...
function clickImportDeviceConfig() {
  ...
  var iRet = WebVideoCtrl.I_ImportDeviceConfig(
    szDeviceIdentify,
    szFileName,
    szDevicePassWord
  );
...
function reconnect(szDeviceIdentify) {
  WebVideoCtrl.I_Reconnect(szDeviceIdentify, {
    ...
    error: function (status, xmlDoc) {
      if (401 == status) {
        // 无插件方案，重启后session已失效，程序执行登出，从已登录设备中删除
        $("#restartDiv").remove();
        clickLogout();
      } else {
        setTimeout(function () {
...
function clickStartUpgrade(szDeviceIdentify) {
  ...
  WebVideoCtrl.I2_StartUpgrade(szDeviceIdentify, szFileName).then(
    function () {
      g_tUpgrade = setInterval(
